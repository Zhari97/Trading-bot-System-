...
def evaluate_signal(signal: dict, candles: list[dict], now: datetime) -> dict:
    ...
    for hours in HORIZONS_H:
        ...
        result["outcomes"][key] = {
            "status": "READY" if datetime.fromtimestamp(horizon_candle["ts"], tz=timezone.utc) + interval >= target else "PENDING",
            "close": close,
...
def _group_stats(rows: list[dict], field: str) -> dict:
    ...
    for row in rows:
        ...
        groups.setdefault(value, {"signals": 0, "ready_1h": 0, "positive_1h": 0, "mean_return_1h_pct": None, "_returns": []})
